            ...
            <div className='min-h-screen flex flex-wrap gap-10 mx-40 my-20'>
                {
                    allLectures.map(item => <div key={item._id} className="flex bg-base-100 shadow-2xl w-full rounded-lg">
                        ...
                        <button disabled={item.status === 'approved' || item.status === 'deny'} className="btn btn-accent mx-3 my-auto" onClick={() => openModal(item, 'deny')}>Deny</button>

                        <button disabled={item.status === 'approved' || item.status === 'deny'} className="btn btn-accent mx-3 my-auto" onClick={() => openModal(item, 'approved')}>Approved</button>
                    </div>)
                }
            </div>
                ...
                <form onSubmit={handleStatusChange} method="dialog" className="modal-box w-11/12 max-w-5xl">
                    ...
                    {selectedItem && (
                        <div className='h-auto'>
                            ...
                            <button className='btn btn-success block ms-auto mt-5'>Submit</button>
                        </div>
                    )}
                    {/* ...your existing JSX code */}
                </form>
